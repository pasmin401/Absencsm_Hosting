...
.photo-avatar { width: 38px; height: 38px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border); transition: transform .2s, border-color .2s; }
...
.checkin-status-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; margin-right: 6px; }
  ...
  <main class="main-content">
    ...
    <div class="attend-grid">
      ...
      <div class="attend-card">
        ...
        <?php if ($canCheckIn): ?>
          <button class="btn btn-primary btn-full" onclick="openModal('checkin')">📍 Check In Now</button>
        <?php else: ?>
      ...
      <div class="attend-card">
        ...
        <?php if ($canCheckOut): ?>
          <button class="btn btn-danger btn-full" onclick="openModal('checkout')">📍 Check Out Now</button>
        <?php else: ?>
      ...
      <div class="attend-card">
        ...
        <?php if ($canOtCheckIn): ?>
          <button class="btn btn-ot btn-full" onclick="openModal('ot_checkin')">⏰ Start Overtime</button>
        <?php else: ?>
          <button class="btn btn-outline btn-full" disabled>
      ...
      <div class="attend-card">
        ...
        <?php if ($canOtCheckOut): ?>
          <button class="btn btn-ot btn-full" onclick="openModal('ot_checkout')">⏰ End Overtime</button>
        <?php else: ?>
          <button class="btn btn-outline btn-full" disabled>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>OT In</th><th>OT Out</th><th>OT Hours</th><th>Photos</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
...
function startCamera() {
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false })
    ...
    .catch(err => showToast('Camera not available: ' + err.message, 'error'));
}
...
async function submitAttendance() {
  ...
  try {
    const res = await fetch('attendance-action.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: currentType, lat, lng, photo })
    });
    const raw = await res.text();
    let data;
    try { data = JSON.parse(raw); }
    catch (e) { showToast('Server error: ' + raw.substring(0, 200), 'error'); document.getElementById('btn-submit').disabled = false; return; }
    if (data.success) {
      showToast(data.message, 'success');
      closeModal();
      setTimeout(() => window.location.href = 'dashboard.php?r=' + Date.now(), 1200);
    } else {
