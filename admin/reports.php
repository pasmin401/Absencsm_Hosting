...
if ($export) {
    ...
    if ($format === 'excel') {
        ...
        fputcsv($out, ['No','Date','Employee','Email','Department','Check In','Check Out','Work Hours','OT Check In','OT Check Out','OT Hours','CI Latitude','CI Longitude','CO Latitude','CO Longitude','OT-CI Latitude','OT-CI Longitude','OT-CO Latitude','OT-CO Longitude','Status']);
        foreach ($records as $i => $r) {
            fputcsv($out, [
                $i + 1,
                $r['work_date'],
...
<style>
.export-box { background: linear-gradient(135deg, #0891b2, #0e7490); border-radius: var(--radius-lg); padding: 24px 28px; color: #fff; margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between; gap: 20px; flex-wrap: wrap; }
...
.summary-item { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 16px; text-align: center; }
  ...
  <main class="main-content">
    ...
    <div class="export-box">
      <div>
        ...
        <p>Download attendance data for the selected date range</p>
      </div>
      <div class="export-btns">
        <a href="?from=<?= urlencode($from) ?>&to=<?= urlencode($to) ?><?= $userId ? '&user_id='.$userId : '' ?>&export=1&format=csv" class="btn btn-white">📄 Download CSV</a>
        <a href="?from=<?= urlencode($from) ?>&to=<?= urlencode($to) ?><?= $userId ? '&user_id='.$userId : '' ?>&export=1&format=excel" class="btn btn-white-outline">📊 Download Excel (.xls)</a>
      </div>
    </div>
    ...
    <form method="GET" class="card" style="padding:20px;margin-bottom:20px">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;align-items:end">
        <div class="form-group" style="margin:0"><label>From Date</label><input type="date" name="from" value="<?= e($from) ?>"></div>
        <div class="form-group" style="margin:0"><label>To Date</label><input type="date" name="to" value="<?= e($to) ?>"></div>
        ...
        <div><button type="submit" class="btn btn-primary btn-full">🔍 Filter</button></div>
        <div><a href="reports.php" class="btn btn-outline btn-full">✕ Clear</a></div>
      </div>
    </form>

    <div class="summary-row">
      <div class="summary-item"><div class="num"><?= $totalPresent ?></div><div class="lbl">Total Records</div></div>
      <div class="summary-item"><div class="num"><?= $uniqueEmployees ?></div><div class="lbl">Unique Employees</div></div>
      <div class="summary-item"><div class="num"><?= $totalOT ?></div><div class="lbl">OT Records</div></div>
      <div class="summary-item"><div class="num"><?= (new DateTime($from))->diff(new DateTime($to))->days + 1 ?></div><div class="lbl">Days in Range</div></div>
    </div>

    <div class="card">
      <div class="card-header">
        <h2>📋 Attendance Records <span class="badge badge-blue" style="margin-left:8px"><?= count($records) ?></span></h2>
        <div style="font-size:.875rem;color:var(--txt2)"><?= date('d M Y', strtotime($from)) ?> → <?= date('d M Y', strtotime($to)) ?></div>
      </div>
      <div class="table-wrap">
        <?php if (empty($records)): ?>
          <div class="empty-state"><div class="icon">📭</div><p>No records found for the selected filters</p></div>
        <?php else: ?>
        <table>
          <thead>
            <tr><th>#</th><th>Date</th><th>Employee</th><th>Dept</th><th>Check In</th><th>Check Out</th><th>Hours</th><th>OT In</th><th>OT Out</th><th>OT Hours</th><th>Location</th><th>Photos</th><th>Status</th></tr>
          </thead>
          <tbody>
            ...
            <tr>
              ...
              <td>
                <?php if ($r['checkin_time']): ?>
                  ...
                  <?php if ($r['checkin_lat']): ?><a class="map-link" href="https://maps.google.com/?q=<?= $r['checkin_lat'] ?>,<?= $r['checkin_lng'] ?>" target="_blank">📍 View</a><?php endif; ?>
                <?php else: ?>—<?php endif; ?>
              </td>
              <td>
                <?php if ($r['checkout_time']): ?>
                  <div style="font-family:'DM Mono',monospace;font-size:.85rem"><?= date('H:i', strtotime($r['checkout_time'])) ?></div>
                  <?php if ($r['checkout_lat']): ?><a class="map-link" href="https://maps.google.com/?q=<?= $r['checkout_lat'] ?>,<?= $r['checkout_lng'] ?>" target="_blank">📍 View</a><?php endif; ?>
                <?php elseif ($r['checkin_time']): ?><span class="badge badge-amber">Active</span>
                <?php else: ?>—<?php endif; ?>
              </td>
              <td><?php $wh = computeWorkHours($r['checkin_time'], $r['checkout_time']); echo $wh ? '<span class="badge badge-green">'.$wh.'</span>' : '—'; ?></td>
              <td>
                <?php if ($r['ot_checkin_time']): ?>
                  ...
                  <?php if ($r['ot_checkin_lat']): ?><a class="map-link" href="https://maps.google.com/?q=<?= $r['ot_checkin_lat'] ?>,<?= $r['ot_checkin_lng'] ?>" target="_blank">📍 View</a><?php endif; ?>
                <?php else: ?>—<?php endif; ?>
              </td>
              <td>
                ...
                <?php elseif ($r['ot_checkin_time']): ?><span class="badge badge-purple">Active OT</span>
                <?php else: ?>—<?php endif; ?>
              </td>
              <td><?php $oth = computeWorkHours($r['ot_checkin_time'], $r['ot_checkout_time']); echo $oth ? '<span class="badge badge-purple">'.$oth.'</span>' : '—'; ?></td>
              ...
              <td>
                <div style="display:flex;gap:4px;flex-wrap:wrap">
                  <?php foreach(['checkin_photo'=>'CI','checkout_photo'=>'CO','ot_checkin_photo'=>'OT↑','ot_checkout_photo'=>'OT↓'] as $col=>$label): ?>
                    <?php if ($r[$col]): ?>
                      <img src="<?= $r[$col] ?>" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid var(--border);cursor:pointer" title="<?= $label ?>"
                           onclick="document.getElementById('lightbox-img').src=this.src; document.getElementById('lightbox').classList.add('open')">
                    <?php endif; ?>
                  <?php endforeach; ?>
                </div>
              </td>
              <td><span class="badge <?= $r['status']==='present' ? 'badge-green' : 'badge-amber' ?>"><?= ucfirst(e($r['status'])) ?></span></td>
            </tr>
          <?php endforeach; ?>
          </tbody>
        </table>
