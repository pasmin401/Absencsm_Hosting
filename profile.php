...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($section === 'password') {
        ...
        if (!password_verify($current, $user['password'])) {
            ...
            updateUser($user['id'], ['password' => password_hash($new, PASSWORD_BCRYPT)]);
...
.photo-upload-zone { border: 2px dashed var(--border2); border-radius: var(--radius); padding: 24px; text-align: center; cursor: pointer; transition: border-color var(--transition), background var(--transition); }
  ...
  <main class="main-content">
    ...
    <div class="profile-header">
      <div class="profile-avatar">
        <?php $pic = $user['profile_pic'] ?? ''; $isDataUrl = $pic && str_starts_with($pic, 'data:'); ?>
        <?php if ($isDataUrl): ?>
      ...
      <div>
        ...
        <div style="display:flex;gap:12px;margin-top:8px;flex-wrap:wrap">
          <?php if ($user['department']): ?><span class="badge badge-blue">🏢 <?= e($user['department']) ?></span><?php endif; ?>
          <?php if ($user['phone']): ?><span class="badge badge-grey">📞 <?= e($user['phone']) ?></span><?php endif; ?>
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div class="card-body">
        <div class="month-stats">
          <div class="month-stat"><div class="num"><?= $totalDays ?></div><div class="label">Days Present</div></div>
          <div class="month-stat"><div class="num"><?= $otDays ?></div><div class="label">OT Days</div></div>
          <div class="month-stat"><div class="num"><?= date('t') - $totalDays ?></div><div class="label">Days Absent</div></div>
        </div>
      </div>
    </div>
      ...
      <div style="display:flex;flex-direction:column;gap:20px">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <form method="POST" id="photo-form">
              ...
              <input type="file" id="profile_pic_input" accept="image/*" style="display:none" onchange="resizeAndUpload(this)">
            </form>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-body" style="display:flex;flex-direction:column;gap:12px">
            <div><div class="text-muted" style="font-size:.78rem;margin-bottom:2px">USER ID</div><div style="font-family:'DM Mono',monospace;font-size:.9rem">#<?= $user['id'] ?></div></div>
            <div><div class="text-muted" style="font-size:.78rem;margin-bottom:2px">ROLE</div><div><?= ucfirst($user['role']) ?></div></div>
            <div>
            ...
            <div><div class="text-muted" style="font-size:.78rem;margin-bottom:2px">JOINED</div><div><?= formatDate($user['created_at']) ?></div></div>
          </div>
        </div>
      </div>
          ...
          <div class="card-body">
            <form method="POST">
              ...
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                <div class="form-group"><label>Username</label><input type="text" name="username" value="<?= e($user['username']) ?>" required maxlength="30"></div>
                <div class="form-group"><label>Email</label><input type="email" name="email" value="<?= e($user['email']) ?>" required></div>
                <div class="form-group"><label>Phone</label><input type="tel" name="phone" value="<?= e($user['phone'] ?? '') ?>" placeholder="+62 xxx"></div>
                <div class="form-group"><label>Department</label><input type="text" name="department" value="<?= e($user['department'] ?? '') ?>" placeholder="e.g. Engineering"></div>
              </div>
            ...
            <form method="POST">
              ...
              <div class="form-group">
                ...
                <div class="input-wrap">
                  <input type="password" name="current_password" id="cur-pwd" class="has-icon" placeholder="Enter current password" required>
              ...
              <div class="form-group">
                ...
                <div class="input-wrap">
                  <input type="password" name="new_password" id="new-pwd" class="has-icon" placeholder="Min. 6 characters" required>
              ...
              <div class="form-group">
                ...
                <div class="input-wrap">
                  <input type="password" name="confirm_password" id="conf-pwd" class="has-icon" placeholder="Repeat new password" required>
